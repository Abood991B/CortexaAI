...
class PlanningConfig:
    """Configuration for advanced planning and reasoning features."""

    # Planning settings
    enable_planning = getattr(settings, 'enable_planning', True)
    planning_engine = getattr(settings, 'planning_engine', 'advanced')  # basic, advanced, expert

    # Reasoning strategies
    enable_chain_of_thought = getattr(settings, 'enable_chain_of_thought', True)
    enable_tree_of_thought = getattr(settings, 'enable_tree_of_thought', True)
    max_reasoning_depth = getattr(settings, 'max_reasoning_depth', 5)
    reasoning_timeout_seconds = getattr(settings, 'reasoning_timeout_seconds', 30)

    # Task decomposition settings
    enable_task_decomposition = getattr(settings, 'enable_task_decomposition', True)
    max_subtasks = getattr(settings, 'max_subtasks', 10)
    task_complexity_threshold = getattr(settings, 'task_complexity_threshold', 0.7)

    # Decision making settings
    decision_strategy = getattr(settings, 'decision_strategy', 'weighted')  # random, weighted, expert
    confidence_threshold = getattr(settings, 'confidence_threshold', 0.8)
    max_decision_options = getattr(settings, 'max_decision_options', 5)

    # Goal management settings
    enable_goal_tracking = getattr(settings, 'enable_goal_tracking', True)
    goal_success_threshold = getattr(settings, 'goal_success_threshold', 0.85)
    max_goal_iterations = getattr(settings, 'max_goal_iterations', 5)

    # Adaptive planning settings
    enable_adaptive_planning = getattr(settings, 'enable_adaptive_planning', True)
    learning_rate = getattr(settings, 'learning_rate', 0.1)
    adaptation_threshold = getattr(settings, 'adaptation_threshold', 0.6)

    # Resource management settings
    max_planning_time = getattr(settings, 'max_planning_time', 60)  # seconds
    planning_memory_limit = getattr(settings, 'planning_memory_limit', 100)  # MB
    concurrent_plans_limit = getattr(settings, 'concurrent_plans_limit', 10)

    # Domain-specific planning settings
    domain_planning_templates = getattr(settings, 'domain_planning_templates', {
        ...
        'data_science': {
            'decomposition_strategy': 'analytical',
            'reasoning_approach': 'hypothesis_driven',
            'validation_required': True
        },
        'general': {
            'decomposition_strategy': 'task_based',
    ...
    enable_planning_caching = getattr(settings, 'enable_planning_caching', True)
    planning_cache_ttl = getattr(settings, 'planning_cache_ttl', 1800)  # 30 minutes
